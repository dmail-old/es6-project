...
export function createResponsePromiseForGet(request){
	...
	promise = filesystem('stat', url).then(function(stat){
		console.log('got stat for', url, stat, stat.isDirectory());
	...
	if( request.method != 'HEAD' ){
		promise = promise.then(function(response){
			console.log('created a read stream for', url, 'got the response', response);
			//console.log('created stream?', Boolean(fs.createReadStream(url)));

			response.body = createBody(fs.createReadStream(url));
			return response;
		});
	}
