...
export var HttpRequest = proto.extend({
	readyState: 'closed',
	method: 'GET',
	url: null,
	headers: {},
	...
	constructor: function(options){
		if( HttpRequest.isPrototypeOf(options) ) return options;
		...
		this.body = DuplexStream.create(this.body);
		this.readyState = 'closed';
		this.currentUrl = this.url;
	},

	clone: function(){
		var cloneRequest = this.create({
			method: this.method,
			url: this.url,
	...
	open: function(){
		if( this.readyState == 'closed' ){
			...
			if( this.cacheMode === 'no-cache' || this.cacheMode === 'no-store' ){
				var searchParams = new platform.global.URLSearchParams(this.url.search);
